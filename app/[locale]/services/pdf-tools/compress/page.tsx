...
import { Minimize2, Upload, Download, Loader2, CheckCircle2, FileWarning } from 'lucide-react';
        ...
        <div className="relative isolate px-6 pt-14 lg:px-8 max-w-7xl mx-auto py-24 sm:py-32 text-white">
            ...
            <div className="max-w-2xl mx-auto">
                <div className="glass p-12 rounded-3xl border border-white/10 text-center space-y-8">
                    {!file ? (
                        <div className="border-2 border-dashed border-white/10 rounded-2xl p-16 relative hover:border-primary transition-all group">
                            <input
                                type="file"
                                accept=".pdf"
                        ...
                        <div className="space-y-8">
                            <div className="flex items-center gap-6 p-6 bg-white/5 rounded-2xl border border-white/10 text-left">
                                <div className="w-16 h-16 bg-rose-500/20 rounded-xl flex items-center justify-center">
                                    <Minimize2 className="w-8 h-8 text-rose-500" />
                                </div>
                                <div className="flex-grow">
                                    <h4 className="font-bold truncate max-w-md">{file.name}</h4>
                                    <p className="text-xs text-gray-500 mt-1">{(file.size / 1024 / 1024).toFixed(2)} {commonT('mb')}</p>
                                </div>
                            ...
                            {status === 'success' ? (
                                ...
                                >
                                    ...
                                    <p className="text-sm opacity-80 mb-6">{pdfT('optimizationResult', { percent: 45, size: (file.size * 0.55 / 1024 / 1024).toFixed(2) })}</p>
                                    <button className="bg-emerald-500 text-white px-8 py-4 rounded-2xl font-bold hover:bg-emerald-600 transition-all flex items-center gap-2 mx-auto shadow-lg">
                                        <Download className="w-5 h-5" /> {pdfT('download')}
                                    </button>
                                </motion.div>
                            ) : (
                                <div className="space-y-6">
                                    <button
                                        ...
                                        className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-5 rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2"
                                    >
                                        {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : <Minimize2 className="w-6 h-6" />}
                                        {loading ? pdfT('processing') : pdfT('compressFiles')}
                                    </button>
                                </div>
                            )}
                        </div>
